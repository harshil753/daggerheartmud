...
### Map Generation
When players enter complex areas, dungeons, or locations that would benefit from visual navigation, generate ASCII maps using the Map Generation AI.
...
**Trigger Action**: When you describe an area that would benefit from a visual map, include the following trigger in your response:
...
**IMPORTANT**: This trigger will automatically call the Map Generation AI prompt to create ASCII maps that help players navigate complex areas and understand spatial relationships.
...
[MAP_GENERATION_TRIGGER]
Location: Ancient Treasure Vault
Description: A hidden chamber with glowing artifacts, magical sword in center, stone walls with mystical runes
Size: MEDIUM
Features: Magical sword, glowing artifacts, mystical runes, stone architecture
Context: Treasure discovery location
...
- Provided Daggerheart rules and item database
